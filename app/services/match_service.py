...
def get_matches(
    ...
    # এখান থেকে date_utc আর্গুমেন্টটি বাদ দেওয়া হয়েছে
...
def _matches_from_db(db, page, page_size, status, stage, group, team_id, sort_by, sort_order):
    # এই ফাংশন থেকে 'date_utc' এর পুরো লজিক ও প্যারামিটার রিমুভ করা হয়েছে
    cache_key = f"matches:{page}:{page_size}:{status}:{stage}:{group}:{team_id}:{sort_by}:{sort_order}"
    cached = cache_get(cache_key)
...
#     if settings.FOOTBALL_DATA_PROVIDER == "api_football":
#         return _matches_from_api(page, page_size, status, group)
#     return _matches_from_db(db, page, page_size, status, stage, group, team_id, date_utc, sort_by, sort_order)
...
#     if date_utc:
#         try:
#             ds = datetime.strptime(date_utc, "%Y-%m-%d").replace(tzinfo=timezone.utc)
#             de = ds.replace(hour=23, minute=59, second=59)
#             query = query.filter(and_(Match.kickoff_utc >= ds, Match.kickoff_utc <= de))
#         except ValueError:
#             pass
