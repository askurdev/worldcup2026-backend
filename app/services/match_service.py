def _matches_from_db(
    ...
    cache_key = (
        f"matches:{page}:{page_size}:{status}:{stage}:"
        f"{group}:{team_id}:{sort_by}:{sort_order}"
    )

    cached = cache_get(cache_key)
    if cached:
        return MatchListResponse(**cached)
    ...
    if group:
        query = (
            query.join(Group, Match.group_id == Group.id)
    ...
    sort_col = (
        Match.kickoff_utc
        if sort_by == "kickoff_utc"
        else Match.match_number
    )
